...
st.markdown("""
    ...
    /* Direct HTML Link Button Styles for Mobile Fix */
...
if not st.session_state.is_registered:
    ...
    # MOBILE FIX: Direct HTML Link for Register
...
# --- Page 2: Predictor Interface ---
    ...
    if st.button("RESULT"):
        if (all(char in "BS" for char in history_raw) and len(history_raw) == 5) and (period_raw.isdigit() and len(period_raw) == 3):
            ...
            # --- 2-Level Sureshot Algorithm (Unchanged) ---
    ...
    # MOBILE FIX: Direct HTML Link for Telegram
